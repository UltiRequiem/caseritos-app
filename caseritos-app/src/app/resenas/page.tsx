...
import { BarChart3, ShoppingBag, User, Star, Search, Filter, Calendar, Download } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuGroup,
...
export default function ReseñasPage() {
  ...
  const calificacionPromedio = reseñas.reduce((total, reseña) => total + reseña.rating, 0) / reseñas.length
          ...
          <div className="hidden md:block w-64 shrink-0">
            <div className="bg-white rounded-xl shadow-sm p-4 sticky top-24">
              <nav className="space-y-1">
                ...
                <Button variant="default" className="w-full justify-start" onClick={() => router.push("/resenas")}>
            ...
            <Card className="p-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6 mb-4">
                <div className="text-center">
                  <div className="text-4xl font-bold text-caserito-green">{calificacionPromedio.toFixed(1)}</div>
                  <div className="flex justify-center mt-2">
                    <StarRating rating={Math.round(calificacionPromedio)} readOnly size={20} />
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">De {reseñas.length} reseñas</p>
                </div>
                <div className="flex-1">
                  {[5, 4, 3, 2, 1].map((rating) => {
                    const count = reseñas.filter((r) => r.rating === rating).length
                ...
                <Card key={reseña.id} className="p-4 overflow-hidden">
                  <div className="flex items-center mb-3">
                    <Avatar className="h-10 w-10 mr-3">
                      <AvatarImage src={reseña.avatar || "/placeholder.svg"} alt={reseña.cliente} />
                      <AvatarFallback>{reseña.cliente.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
