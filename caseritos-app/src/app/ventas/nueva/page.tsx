...
import { Input, Textarea, Button, Toast, Modal, ImageUploader } from "@/components/ui-components"
import { ArrowLeft, Plus, Minus, Trash2, Loader2, Copy, CheckCircle2, AlertTriangle, Camera } from "lucide-react"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (Math.random() > 0.8) {
        ...
        const ventaId = `v-${Date.now().toString(36)}-${Math.random().toString(36).substring(2, 7)}`
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 container mx-auto p-4 pb-20 md:pb-4">
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex items-center justify-between p-3 bg-gray-50">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {producto.cantidad} {producto.unidad} x ${producto.precio.toFixed(2)}
                        </p>
                      </div>
                      <div className="flex items-center">
                        <p className="font-bold mr-4">${producto.subtotal.toFixed(2)}</p>
                      ...
                      <div className="p-3 border-t bg-gray-50">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2">
                          {producto.imagenes.map((img, imgIndex) => (
                            <div key={imgIndex} className="relative h-16 w-16 flex-shrink-0">
                              <Image
                  ...
                  <div className="p-4">
                            ...
                            <SelectContent>
                              {productosDisponibles.map((producto) => (
                                <SelectItem key={producto.id} value={producto.id}>
                                  {producto.nombre} - ${producto.precio.toFixed(2)}/{producto.unidad}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                    ...
                    <TabsContent value="detalles" className="mt-0 space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <ImageUploader onChange={handleImageUpload} maxSize={5} />
                      </div>
            ...
            <Button type="submit" className="w-full" loading={saving} disabled={saving || productos.length === 0}>
        ...
        <div className="fixed inset-0 bg-black/50 flex flex-col items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full text-center">
            ...
            <p className="text-muted-foreground mb-4">
              Estamos verificando que los datos e imágenes de la venta sean coherentes...
            </p>
      ...
      <Modal
        isOpen={showModal}
        onClose={() => {
          if (modalType === "success" && enlaceUnico) {
            // No cerrar el modal si es éxito y hay enlace
          } else {
            setShowModal(false)
          }
        }}
      ...
      >
        <div className="text-center">
          <h3 className="text-lg font-medium mb-2">
            {modalType === "success" ? "¡Venta validada correctamente!" : "No se pudo validar la venta"}
          </h3>

          {modalType === "success" && enlaceUnico ? (
            <>
              <p className="text-muted-foreground mb-4">
                La venta ha sido registrada y validada. Comparte este enlace único con tu cliente para que pueda
                confirmar la recepción y dejar su reseña.
              </p>

              <div className="mb-4 p-3 bg-gray-100 rounded-md flex items-center">
                <p className="text-sm truncate flex-1">{enlaceUnico}</p>
                <Button onClick={copyLink} variant="ghost" size="sm" className="ml-2">
            ...
            <>
              <p className="text-muted-foreground mb-4">
                La IA ha detectado inconsistencias en los datos o imágenes de la venta. Por favor, revisa la información
