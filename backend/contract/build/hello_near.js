...
let Registry = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec(_class = (_class2 = class Registry {
  records = new Vector("r");
  add_record({
    did1,
    hash,
    did2
    ...
    log(`Added record: ${entry}`);
  }
  get_records() {
    return this.records.toArray();
  }
}, _applyDecoratedDescriptor(_class2.prototype, "add_record", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "add_record"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_records", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "get_records"), _class2.prototype), _class2)) || _class);
function get_records() {
  const _state = Registry._getState();
  if (!_state && Registry._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = Registry._create();
  if (_state) {
    Registry._reconstruct(_contract, _state);
  }
  const _args = Registry._getArgs();
  const _result = _contract.get_records(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Registry._serialize(_result, true));
}
function add_record() {
  const _state = Registry._getState();
  if (!_state && Registry._requireInit()) {
  ...
  const _args = Registry._getArgs();
  const _result = _contract.add_record(_args);
  Registry._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Registry._serialize(_result, true));
}

export { add_record, get_records };
