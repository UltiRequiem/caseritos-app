...
import { CredentialRequestValidation } from './credential-request-validation.dto';
...
export class CredentialRequestDto {
  @ApiProperty({
    required: false,
    description: "(draft 13+) REQUIRED when an Authorization Details of type openid_credential was "
  ...
  @ApiProperty({
    required: false,
    description: "(draft 13+) REQUIRED if a credential_identifiers parameter was not returned "
  ...
  @ApiProperty({
    required: false,
    description: "(draft10 & draft11) REQUIRED. Format of the Credential to be issued. This Credential "
  ...
  @ApiProperty({
    required: false,
    description: "List of string containing credential types to issue. It is not necessary if using "
      + "credential_configuration_id or credential_identifier for draft 13+.",
  ...
  @ApiProperty({
    required: true,
    description: "Object providing a single proof of possession of the cryptographic "
  ...
  @ApiProperty({
    required: false,
    description: "Not supported. Object providing one or more proof of possessions of the "
  ...
  isValid(xCorrelationId: string): CredentialRequestValidation {
    ...
    if (!isProofEmpty && proof?.proof_type !== "jwt") {
      isValid = false;
      messageBuilder.push("Request field 'proof' must contain 'proof_type' as 'jwt'.");
    }
